...
#define STACK_MAX 32
{
  ...
  for (c = 0; c < b->params_len; c++)
    ...
    TOP:
      pnum =
	lp_param_name (lp_mod_name ("disksim_cachedev"), b->params[c]->name);
	...
	case DISKSIM_CACHEDEV_CACHE_SIZE:
	  {
		...
		if (result->validmap || result->dirtymap)
		  {
		    fprintf (stderr,
			     "*** ignoring repeat cache size definition.\n");
		  }
		else
		  {
		    result->size = i;
		    result->validmap =
		      (bitstr_t *)
		      DISKSIM_malloc (bitstr_size (result->size));
		    result->dirtymap =
		      (bitstr_t *)
		      DISKSIM_malloc (bitstr_size (result->size));
		    ASSERT ((result->validmap != NULL)
			    && (result->dirtymap != NULL));
		  }
	      }
	  }
	  break;
