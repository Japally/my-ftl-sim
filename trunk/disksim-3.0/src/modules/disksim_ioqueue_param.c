...
#define STACK_MAX 32
{
  ...
  for (c = 0; c < b->params_len; c++)
    ...
    TOP:
      pnum =
	lp_param_name (lp_mod_name ("disksim_ioqueue"), b->params[c]->name);
	...
	case DISKSIM_IOQUEUE_SCHEDULING_TIMEOUT_SCHEME:
	  {
	    if (disksim->verbosity > 0)
	      fprintf (stderr,
		       "** warning: ignoring \"Scheduling timeout scheme\" \n");
	  }
	  break;
