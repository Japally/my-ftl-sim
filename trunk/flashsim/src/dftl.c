  ...
  for (i = 0; i < PAGE_NUM_PER_BLK; i++) 
  {
    ...
    if(valid_flag == 1)
    {
        ...
        for (j = 0; j < valid_sect_num; j++) {
          ...
          if(nand_blk[victim_blk_no].page_status[i] == 1)  //对于地址转换页分开对待
          {                       
            mapdir[(copy_lsn[s]/SECT_NUM_PER_PAGE)].ppn  = BLK_PAGE_NO_SECT(SECTOR(free_blk_no[small], free_page_no[small]));  //对GTD进行更新
            opagemap[copy_lsn[s]/SECT_NUM_PER_PAGE].ppn = BLK_PAGE_NO_SECT(SECTOR(free_blk_no[small], free_page_no[small]));  //对自定义的查找表也进行更新
